    ...
    <section class="mt-7">
        <div class="flex flex-col-reverse sm:flex-row  sm:justify-between gap-3">
            <div class="bg-white p-5 w-full h-auto sm:w-2/3 border border-gray-200 rounded-lg shadow">
                {{-- CONTENIDO PENDIENTE --}}
                <h3 class="text-lg text-teal-700 mb-2 font-semibold border-b border-gray-300">Actividades pendientes</h3>
                <div class="h-[28rem] overflow-y-auto pr-1">
                    @foreach ($pendientes as $actividad)
                        <x-card-actividad iconoA="question" color1="blue"
                            nombreActividad="{{ $actividad['nom_actividad'] }}" tipo="{{ $actividad['tipo'] }}"
                            fecha="{{ $actividad['fecha_fin'] }}" iconoEntregable="pending" color2="orange"
                            link="{{ route('alumno.responder-actividad', $actividad['pk_actividad']) }}" />
                    @endforeach
                </div>
            </div>
            <div class="flex flex-col w-full sm:w-1/3 ">
                ...
                <a href="{{ route('alumno.lista-actividades') }}" class="mt-4">
                    <div class=" bg-white p-4 h-auto border border-gray-300 rounded-lg shadow">
                        <div class="flex justify-between">
                            <div class="w-5/6 text-justify">
                                <h3 class="text-lg text-teal-700 font-semibold">Mi lista de actividades</h3>
                                <span class="text-sm text-gray-600 font-normal">Visualiza las actividades asignadas y su estado</span>
                            </div>
                            <div class="flex justify-center items-center w-1/6 pl-1">
                                <svg width="48" height="48" class="text-center" viewBox="0 0 48 48" fill="none"
                                    stroke="#2B877A" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">
