...
<section>
    ...
    <span class="text-sm sm:text-lg text-gray-500">Consulta en este apartado todas las actividades asignadas.</span>
</section>
        ...
        <div class="flex justify-between p-2 gap-5 border-b border-gray-200">
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('alumno.lista-actividades', ['filtro' => 'pendientes']) }}"
                    class="text-sm sm:text-lg font-medium border-b-2 {{ $filtro == 'pendientes' ? 'border-teal-700 text-teal-700' : 'border-transparent text-gray-700 hover:border-teal-700' }}">
                    Pendientes
                </a>
                <a href="{{ route('alumno.lista-actividades', ['filtro' => 'entregadas']) }}"
                    class="text-sm sm:text-lg font-medium border-b-2 {{ $filtro == 'entregadas' ? 'border-teal-700 text-teal-700' : 'border-transparent text-gray-700 hover:border-teal-700' }}">
                    Terminadas
                </a>
                <a href="{{ route('alumno.lista-actividades', ['filtro' => 'no_entregadas']) }}"
                    class="text-sm sm:text-lg font-medium border-b-2 {{ $filtro == 'no_entregadas' ? 'border-teal-700 text-teal-700' : 'border-transparent text-gray-700 hover:border-teal-700' }}">
                    No entregadas
                </a>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    input.addEventListener('input', () => {
        ...
        cards.forEach(card => {
            const nombre = card.querySelector('[data-nombre-actividad]').textContent.toLowerCase();
            const tipo = card.querySelector('[data-tipo-actividad]').textContent.toLowerCase();
            card.style.display = (nombre.includes(query) || tipo.includes(query)) ? '' : 'none';
        });
    });
