                    ...
                    <tr
                        ...
                        <td class="py-2 px-2 md:px-4 border-b border-gray-200">
                            <div class="flex items-center space-x-3">
                                ...
                                <img src="{{ $coordinador->img_user ? asset('storage/' . $coordinador->img_user) : asset('img/default.jpg') }}"
                                    alt="coordinador" class="w-10 h-10 rounded-full object-cover border" />
                                <div>
                                    <span
                                        class="text-teal-600 sm:text-gray-800 text-xl sm:text-base font-semibold sm:font-medium">
                                        {{ $coordinador->nombres }}
                        ...
                        <td class="py-2 px-2 md:px-4 border-b border-gray-200">
                            <div class="flex sm:flex-none justify-center items-center gap-2 sm:gap-0">
                                <span class="sm:hidden">Permiso:</span>
                                <select name="tipo_usuario"
                                    ...
                                    <option value="2" {{ $coordinador->fk_tipo_usuario == 2 ? 'selected' : '' }}>
                                        Docente</option>
                                    <option value="3" {{ $coordinador->fk_tipo_usuario == 3 ? 'selected' : '' }}>
                                        Coordinador</option>
                                </select>
                            </div>
                        </td>
                        <td class="py-2 px-2 md:px-4 border-b border-gray-100">
                            <div class="flex items-center justify-center gap-2">
                                @if ($coordinador->deleted_at)
                                    <form action="{{ route('coordinador.restaurar', $coordinador->pk_usuario) }}"
                                        method="POST">
                                        @csrf
                                        <button type="submit" data-swal-form data-swal-title="¿Habilitar Coordinador?"
                                ...
                                @else
                                    <a href="{{ route('coordinador.detalle', $coordinador->pk_usuario) }}"
                                        class="text-cyan-600 hover:text-cyan-800" title="Detalles">Detalles</a>
                                    <a href="{{ route('coordinador.cargar', $coordinador->pk_usuario) }}"
                                        class="text-green-600 hover:text-green-800" title="Editar">Editar</a>

                                    <form action="{{ route('coordinador.eliminar', $coordinador->pk_usuario) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" data-swal-form
